...
def create_brain(config: Dict[str, Any], silent: bool = False):
    ...
    vocab_size = language_config.get('vocabulary', {}).get('max_size', 10000)  # Organism max vocab
    ...
    brain = OrganismBrain(
        input_dim=brain_config.get('input_dim', 30),
...
def validate_architecture_config(config: Dict[str, Any], strict: bool = False) -> bool:
    ...
    checks = [
        ...
        ('neural.language_model.teacher.vocab_size', teacher_config.get('vocab_size'), 50000),  # Teacher knows full distilled vocab
    ]
